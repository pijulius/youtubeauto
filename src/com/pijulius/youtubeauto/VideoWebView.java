...
public class VideoWebView extends VideoEnabledWebView {
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		if (event.getAction() == MotionEvent.ACTION_UP) {
			if (Math.abs(lastDownX - event.getX()) < TOUCH_TOLERANCE
					&& Math.abs(lastDownY - event.getY()) < TOUCH_TOLERANCE)
				showKeyboardIfInput();
		}
	...
	public void discoverVideoElements() {
		loadUrl("javascript: function waitForElementToDisplay(selector, time, fn) {\n"
				+ "var el = document.querySelector(selector);" + "if(el!=null) {" + "            fn(el);"
				+ "            return;" + "        }" + "        else {" + "            setTimeout(function() {"
				+ "                waitForElementToDisplay(selector, time, fn);" + "            }, time);\n"
	...
	public void playVideoIfPausedFlag() {
		loadUrl("javascript: " + "if(window.pausedDueToMinimize) {"
				+ "var el = document.getElementsByTagName('video')[0];" + "    el.play();"
	...
	public void findAndClickFirstVideo() {
		String s = ("javascript: var el = document.querySelector(\"a[href^='/watch'\"); " + "el.click();" + "");
		loadUrl(s);
	}
	...
	public void showKeyboardIfInput() {
		String s = "javascript:setTimeout(function() {" + "if(document.activeElement instanceof HTMLInputElement) {"
				...
				+ "window.nativecallbacks.showKeyboard(document.activeElement.innerText);" + "} else {" + "" + "}"
	...
	public void enterEditText(String s) {
		String js = "var changeEvent = new Event('change'); " + "if(document.activeElement.isContentEditable)" + "{"
				+ "document.activeElement.innerText = \"" + s + "\";" + "}" + "" + "else " + "{"
