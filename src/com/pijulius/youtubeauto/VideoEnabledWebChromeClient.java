 ...
 * playVideo video. It works with old VideoView-s as well as the new
...
public class VideoEnabledWebChromeClient extends WebChromeClient
	...
	private boolean isVideoFullscreen; // Indicates if the video is being displayed using a custom view (typically
										// full-screen)
	...
	/**
	 * Never use this constructor alone. This constructor allows this class to be
	 * defined as an inline inner class in which the user can override methods
	 */
	 ...
	 *
	 * @param activityNonVideoView A View in the activity's layout that contains
	 *                             every other view that should be hidden when the
	 *                             video goes full-screen.
	 * @param activityVideoView    A ViewGroup in the activity's layout that will
	 *                             display the video. Typically you would like this
	 ...
	 *                             (typically only used in API level <11). Must be
	 *                             already inflated and without a parent view.
	 */
	 ...
	 *                             (typically only used in API level <11). Must be
	 *                             already inflated and without a parent view.
	 * @param webView              The owner VideoEnabledWebView. Passing it will
	 *                             enable the VideoEnabledWebChromeClient to detect
	 *                             the HTML5 video ended event and exit full-screen.
	 *                             Note: The web page must only contain one video
	 *                             tag in order for the HTML5 video ended event to
	 *                             work. This could be improved if needed (see
	 *                             Javascript code).
	 */
	public VideoEnabledWebChromeClient(View activityNonVideoView, ViewGroup activityVideoView, View loadingView,
	 ...
	 * full-screen)
	 *
	 * @return true it the video is being displayed using a custom view (typically
	 *         full-screen)
	 */
	...
	public void onShowCustomView(View view, CustomViewCallback callback) {
		Log.d(getClass().getName(),
				"onShowCustomView() called with: view = [" + view + "], callback = [" + callback + "]");
		if (view instanceof FrameLayout) {
			...
			} else // Usually android.webkit.HTML5VideoFullScreen$VideoSurfaceView, sometimes
	...
	private void handleVideoStretching() {
		...
		if (videoStretchingEnabled && isVideoFullscreen()) {
			js += "_ytrp_html5_video.style.objectFit='fill';"
					+ " _ytrp_html5_video.style.height=window.innerHeight+'px';"
					+ "_ytrp_html5_video.style.width=window.innerWidth+'px';" + "_ytrp_html5_video.style.top=0;";
		} else {
	...
	@Override
	public void onShowCustomView(View view, int requestedOrientation, CustomViewCallback callback) // Only available in
																									// API level 14+
	{
	...
	public void onHideCustomView() {
		...
		// VideoView (typically API level <11)
		// This method must be manually (internally) called on video end in the case of
		// HTML5VideoFullScreen (typically API level 11+) because it's not always called
		// automatically
		// This method must be manually (internally) called on back key press (from this
...
//                js += "}";
	...
	@Override
	public View getVideoLoadingProgressView() // Video will start loading, only called in the case of VideoView
	...
	@Override
	public void onPrepared(MediaPlayer mp) // Video will start playing, only called in the case of VideoView (typically
											// API level <11)
	{
		if (loadingView != null) {
	...
	@Override
	public void onCompletion(MediaPlayer mp) // Video finished playing, only called in the case of VideoView (typically
												// API level <11)
	{
		onHideCustomView();
	}
	...
	@Override
	public boolean onError(MediaPlayer mp, int what, int extra) // Error while playing video, only called in the case of
																// VideoView (typically API level <11)
	{
	 ...
	 *
	 * @return Returns true if the event was handled, and false if it is not (video
	 *         view is not visible)
	 */
